...
//
// kv_file_system_ops: Async-friendly disk I/O for KV-cache block offloading.
//
...
static bool fail_write(int fd, const std::string& tmp_path,
  ...
  std::cerr << "[kv_file_system_ops] " << op << " failed for " << tmp_path
            << ": " << std::strerror(err) << "\n";
  if (fd >= 0) close(fd);
...
static bool fail_read(int fd, const std::string& src_path,
                       const char* op, int err) {
  std::cerr << "[kv_file_system_ops] " << op << " failed for " << src_path
            << ": " << std::strerror(err) << "\n";
  if (fd >= 0) close(fd);
  return false;
}
...
static bool write_block(const uint8_t* src,
  ...
  if (!parent.empty()) {
    ...
    if (ec) {
      std::cerr << "[kv_file_system_ops] create_directories failed for "
...
static bool read_block(uint8_t* dst,
  ...
  while (total < static_cast<size_t>(block_size)) {
    ...
    if (n == 0) {
      std::cerr << "[kv_file_system_ops] unexpected EOF in " << src_path
